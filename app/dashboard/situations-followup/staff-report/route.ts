...
import { buildAdminFollowupStaffDailyReportHtml } from "@/lib/admin-followup-staff-report-html";
...
export async function GET(req: Request): Promise<Response> {
  ...
  const rows = all.filter((row) => row.exam_date === examDate);
  const html = buildAdminFollowupStaffDailyReportHtml({
    examDate,
    rows,
